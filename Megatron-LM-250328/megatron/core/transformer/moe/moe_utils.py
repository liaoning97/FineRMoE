...
#        print(f"sorted_indices.shape: {sorted_indices.shape}")
        ...
        # token_indices: [num_experts, num_tokens]
        # sorted_indices: [num_tokens] num_tokens = seq_len * top_k
...
def unpermute(
    ...
    Args:
        ...
        drop_and_pad (bool, optional): Whether or not the token dispatcher uses token-drop
                                       and pads the number of tokens to the expert capacity.
        G_I (int): fine-grained granualarity at the intermediate dimension.
        G_O (int): fine-grained granualarity at the output dimension.
    ...
    if G_O == 1:
        ...
        output_hideen_size = restore_shape[1] // G_O
        seq_len = restore_shape[0]
        topk = permuted_tokens.shape[0] // seq_len
        for i in range(G_O):
            output_tokens[:, i*output_hideen_size:(i+1)*output_hideen_size].scatter_add_(0, sorted_indices[(i*topk//G_O)*seq_len:((i+1)*topk//G_O)*seq_len].unsqueeze(1).expand(-1, output_hideen_size), permuted_tokens[(i*topk//G_O)*seq_len:((i+1)*topk//G_O)*seq_len])
...
def keep_max_and_mask(tensor):
    """
    Generates the mask for a 3D tensor that preserves the max element in the last dimension
    """
...
def finermoe_router(
    ...
    """
    1. generates the mask for within each add-sum group in the sparse sum layer.
    2. genearates the mask for the experts corresponding to the selected vector in the sparse concatenation layer.
    3. AND operation between the two masks to get the final mask for topk selection.
    
    Args:
        scores (torch.Tensor): logits tensor output from the router network.
        num_tokens (int): number of tokens.
        num_experts (int): number of experts.
        G_I (int): fine-grained granualarity at the intermediate dimension.
        G_O (int): fine-grained granualarity at the output dimension.
        topk (int): The number of activated experts for each token.
        R_I (int): expansion rate at the intermediate dimension.
        R_O (int): expansion rate at the output dimension.
    ...
    _, add_shard_top_indices = torch.topk(concat_shard_scores, topk // G_O, dim=-1)
    activate_add_shard_mask = torch.zeros_like(concat_shard_scores, dtype=torch.bool) # [num_token, R_O * G_O, R_I * G_I]
    activate_add_shard_mask.scatter_(-1, add_shard_top_indices, True) # [num_token, R_O * G_O, R_I * G_I]
    ...
    activate_concat_shard_mask = keep_max_and_mask(concat_shard_scores) 
    activate_concat_shard_mask = activate_concat_shard_mask.view(num_tokens, -1) # [num_token, R_O * G_O]
    activate_concat_shard_mask = activate_concat_shard_mask.unsqueeze(-1).repeat(1,1,R_I*G_I) # [num_token, R_O * G_O, R_I * G_I]
...
def topk_softmax_with_capacity(
    ...
    if score_function == "softmax":
        if use_pre_softmax:  # here, only when the number of activated experts is 1
            ...
            if G_O == 1:
                ...
                scores, top_indices = finermoe_router(logits, num_tokens, num_experts, G_I, G_O, topk, R_I, R_O)
                probs = torch.softmax(scores, dim=-1, dtype=torch.float32).type_as(logits)
    ...
    topk_masked_gates = torch.zeros_like(logits).scatter(1, top_indices, probs) # shape with [num_tokens, num_experts], value with 0 or routing score (for selected)
    topk_map = torch.zeros_like(logits).int().scatter(1, top_indices, 1).bool() # shape with [num_tokens, num_experts], value with False or True (for selected)
