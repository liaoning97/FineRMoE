...
class TEGroupedMLP(MegatronModule):
    ...
    def __init__(self, num_local_experts, config: TransformerConfig, submodules: MLPSubmodules):
        ...
        self.linear_fc2 = build_module(
            ...
            self.config.hidden_size // self.config.G_O,
            config=self.config,
    ...
    def forward(
        ...
        intermediate_parallel, bias_parallel = self.linear_fc1(
            permuted_local_hidden_states, tokens_per_expert
        ) # permuted_local_hidden_states [num_token, hidden_size]

        if self.config.bias_activation_fusion: # True
        ...
        # intermediate_parallel [num_token, moe_intermediate_size]
        output, output_bias = self.linear_fc2(intermediate_parallel, tokens_per_expert)
        # output [num_token, hidden_size]
