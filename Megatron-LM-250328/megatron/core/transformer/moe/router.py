...
class Concat_Out_Proj(ABC, MegatronModule):
    """The linear projection after the multi-expert concatenation"""

    def __init__(self, config: TransformerConfig) -> None:
        """
        Initialize the Concat_Out_Proj module.
        ...
        self.weight = torch.nn.Parameter(
            torch.empty((self.config.hidden_size, self.config.hidden_size), dtype=torch.float32)
        )
    ...
    def forward(self, input: torch.Tensor):
        ...
        output = torch.nn.functional.linear(input.to(input_dtype), self.weight.to(input_dtype))
        return output
...
class TopKRouter(Router):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        self.routing_type = self.config.moe_router_load_balancing_type  # seq_aux_loss
        self.score_function = self.config.moe_router_score_function  # softmax
        self.input_jitter = None

        self.enable_expert_bias = self.config.moe_router_enable_expert_bias  # False
    ...
    def aux_loss_load_balancing(self, logits: torch.Tensor):
        ...
        if self.training and torch.is_grad_enabled():
            ...
            loss_func = switch_load_balancing_loss_func
            extra_args = {}
    ...
    def seq_aux_loss_load_balancing(self, logits: torch.Tensor, bsz: int, seq_length: int):
        ...
        # probs: shape with [num_tokens, num_experts], value with 0 or routing score (for selected)
        # routing_map: shape with [num_tokens, num_experts], value with False or True (for selected)
        # tokens_per_expert: shape with [num_experts], value with number of tokens that are routed to each expert
        if self.training and torch.is_grad_enabled():
            # Apply sequence-auxiliary load balancing loss
            scores = self.compute_routing_scores_for_aux_loss(logits)
            loss_func = sequence_load_balancing_loss_func
            extra_args = {}
            aux_loss_func = partial(
                loss_func,
                probs=scores,
    ...
    def apply_input_jitter(self, input: torch.Tensor):
        ...
        """
        if self.config.moe_input_jitter_eps is not None:  # None
    ...
    def routing(self, logits: torch.Tensor):
        ...
        # Apply Z-Loss
        logits = self.apply_z_loss(logits)  # direct return without applying z-loss
        ...
        elif self.routing_type == "seq_aux_loss":  # here
        ...
        if self.enable_expert_bias and torch.is_grad_enabled():  # false
            with torch.no_grad():
    ...
    def forward(self, input: torch.Tensor):
        ...
        logits = self.gating(input) # [seq_len, 1, num_experts]

        scores, routing_map = self.routing(logits)
        # scores [seq_len, num_experts]
        # routing_map [seq_len, num_experts]
