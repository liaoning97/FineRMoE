...
# 稀疏专家数量：num_experts
# 稀疏专家维度：moe_ffn_hidden_size
# 激活专家数量：moe_router_topk
# 共享专家维度：moe_shared_expert_intermediate_size
...
def compute_weight_and_optimizer_memory(args, verbose=False):
    ...
    if args.num_experts is None: # Dense Model
        ...
        activated_num_parameters_in_transformer_layers = num_parameters_in_transformer_layers
    else: # MoE Model
        num_parameters_in_transformer_layers = (
            2
            * args.num_layers
            ...
            * (
                ...
                # Shared Expert. if args.moe_shared_expert_intermediate_size == 0, then the element is 0.
                + (((args.moe_shared_expert_intermediate_size / args.hidden_size) * gated_linear_multiplier) if args.moe_shared_expert_intermediate_size else 0)
                # Sparse Expert UP.
                + ((args.moe_ffn_hidden_size / args.hidden_size) * args.num_experts * (gated_linear_multiplier - 1/2))
                # Sparse Expert Down.
                + ((args.moe_ffn_hidden_size / args.hidden_size / args.G_O) * args.num_experts * 1/2)
                # Router.
                + (args.num_experts / args.hidden_size)
                # MoE-Concat-Proj.
                + ((args.hidden_size / args.hidden_size) if args.moe_concat_proj else 0)
                # Transformer layernorms.
                + (2 / args.hidden_size)
                # Final layernorm.
                + (1 / (args.num_layers * args.hidden_size))
        ...
        activated_num_parameters_in_transformer_layers = (
            2
            * args.num_layers
            * args.hidden_size
            * args.hidden_size
            * (
                ...
                + (((args.moe_shared_expert_intermediate_size / args.hidden_size) * gated_linear_multiplier) if args.moe_shared_expert_intermediate_size else 0)
                # Sparse Expert UP.
                + ((args.moe_ffn_hidden_size / args.hidden_size) * args.moe_router_topk * (gated_linear_multiplier - 1/2))
                # Sparse Expert Down.
                + ((args.moe_ffn_hidden_size / args.hidden_size / args.G_O) * args.moe_router_topk * 1/2)
                # Router.
                + (args.num_experts / args.hidden_size)
                # MoE-Concat-Proj.
                + ((args.hidden_size / args.hidden_size) if args.moe_concat_proj else 0)
    ...
    num_activate_parameters = activated_num_parameters_in_transformer_layers + num_parameters_in_embedding_layers
    if verbose:
        print(
            ...
            f"{activated_num_parameters_in_transformer_layers / 10**9: .2f}"
        )
        print(
        ...
        print(f"Total number of activated parameters in billions: {num_activate_parameters / 10**9:.2f}")
