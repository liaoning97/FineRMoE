...
def get_moe_module_spec(
    ...
    if moe_grouped_gemm:
        ## use GroupedMLP
        if use_te and TEColumnParallelGroupedLinear is not None and not moe_use_legacy_grouped_gemm: # Here
            ## use TEGroupedLinear
...
def get_gpt_layer_with_transformer_engine_spec(
    ...
    moe_use_legacy_grouped_gemm: Optional[bool] = False,
    use_shared_expert_gate: Optional[bool] = True,
) -> ModuleSpec:
...
def get_gpt_decoder_block_spec(
    ...
    dense_layer_spec = (
        get_gpt_layer_with_transformer_engine_spec(
            ...
            use_shared_expert_gate=config.use_shared_expert_gate
        )
        if use_transformer_engine
    ...
    moe_layer_spec = (
        get_gpt_layer_with_transformer_engine_spec(
            ...
            moe_grouped_gemm=config.moe_grouped_gemm, # True
            qk_layernorm=config.qk_layernorm, # False
            moe_use_legacy_grouped_gemm=config.moe_use_legacy_grouped_gemm, # False
            use_shared_expert_gate=config.use_shared_expert_gate
        )
        if use_transformer_engine
        else get_gpt_layer_local_spec(
