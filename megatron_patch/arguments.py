...
def get_patch_args(parser):
    ...
    group.add_argument(
        "--freeze-attention",
        action="store_true",
        help="Whether to freeze weights in attention",
    )
    group.add_argument(
        "--freeze-shared-expert",
        action="store_true",
        help="Whether to freeze weights in shared experts",
    )
    group.add_argument(
        "--freeze-list",
        type=str,
        default="",  # Default: no freeze.
        help="keywords of items to freeze, Eg. 'embedding+attention+mlp.shared_experts'",
    )
    group.add_argument(
        "--only-train-router",
        action="store_true",
        help="Whether to only train weights in router",
    )
    group.add_argument(
        "--only-train-list",
        type=str,
        default="all",  # Default: all weights should be trained
        help="keywords of items to only train, Eg. 'router+mlp.experts+layernorm'",
    )
    #################################
    # also set in megatron_patch/model/qwen2_moe/transformer_config.py
    group.add_argument(
        "--G-O",
        type=int,
        default=1,
        help="Fine-grained granularity at the output dimension.",
    )
    #################################
    # also set in megatron_patch/model/qwen2_moe/transformer_config.py
    group.add_argument(
        ...
        type=int,
        default=1,
        help="Fine-grained granularity at the intermediate dimension.",
    )
    #################################
    # also set in megatron_patch/model/qwen2_moe/transformer_config.py
    group.add_argument(
        ...
        type=int,
        default=0,
        help="Expansion rate at the intermediate dimention.",
    )
    #################################
    # also set in megatron_patch/model/qwen2_moe/transformer_config.py
    group.add_argument(
        ...
        type=int,
        default=0,
        help="Expansion rate at the output dimention.",
    )
    #################################
    # Whether to use the output linear projection after multi-expert concat
    group.add_argument(
        "--moe-concat-proj",
        action="store_true",
        help="Whether to use the linear projection after multi-expert concat",
    )
    group.add_argument(
        "--moe-aux-loss-coeff-scheduler",
        type=str,
        default=None,
        help="scheduler of moe aux loss coeff",
    )
