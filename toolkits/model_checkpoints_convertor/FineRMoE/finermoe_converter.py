...
from FineRMoE.pretrain import model_provider
...
def load_megatron_model(args):
    ...
    import glob
    if bool(glob.glob(f"{args.hf_ckpt_path}/**/*.py", recursive=True)):
    ...
    if (
        ...
        for tp_rank in range(args.tensor_model_parallel_size):
            for ep_rank in range(tp_rank, args.expert_model_parallel_size, args.tensor_model_parallel_size):
                for pp_rank in range(args.pipeline_model_parallel_size):
                    ...
                    checkpoint_name = get_checkpoint_name(model_path, iteration, release, 
                            pipeline_parallel=args.pipeline_model_parallel_size>1, 
                            tensor_rank=tp_rank, 
                            pipeline_rank=pp_rank, 
                            expert_parallel=args.expert_model_parallel_size>1,
                            expert_rank=ep_rank)
                    ...
                    for k, v in split_state.items():
                        ...
                        try:
                            ...
                            if 'linear_proj' in k or 'shared_experts.linear_fc1' in k or 'shared_experts.linear_fc2' in k or "linear_qkv" in k:
                                if ep_rank == tp_rank:
        ...
        for k, v in mid_state.items():
            if 'extra_state' in k:
                continue
            elif not isinstance(v[0], torch.Tensor) or 'router' in k or 'gate' in k or 'concat_proj' in k:
                target_v = v[0]
...
def convert_from_megatron_to_transformers(mgmodel, hfmodel, args):
    if args.fp16:
    ...
    with torch.no_grad():
        ...
        for mglayer, hflayer in zip(mgmodel.decoder.layers, hfmodel.model.layers):
            ...
            if args.num_experts is None:
                ...
                if hasattr(mglayer.mlp, "concat_proj"):
                    print("converting concat_proj")
                    hflayer.mlp.concat_proj.weight.copy_(mglayer.mlp.concat_proj.weight)
                ...
                if hasattr(hflayer.mlp, 'shared_expert') and hflayer.mlp.shared_expert is not None:
                    if getattr(hflayer.mlp, "shared_expert_gate", None):
        ...
        if args.untie_embeddings_and_output_weights:
            ...
            if hasattr(hfmodel.lm_head, 'weight'):
                hfmodel.lm_head.weight.copy_(mgmodel.embedding.word_embeddings.weight)

def upcycle_checkpoint_from_transformers_to_megatron(hfmodel: Qwen2MoeForCausalLM, mgmodel: GPTModel, args):
    ...
    with torch.no_grad():
        ...
        for layer_idx, (mglayer, hflayer) in tqdm(enumerate(zip(mgmodel.decoder.layers, hfmodel.model.layers)), total=len(mgmodel.decoder.layers)):
            ...
            ########################################
            # Step-1: Initialize shared expert
            ########################################
            print(f"select_shared_expert_method: {args.select_shared_expert_method}, Layer id: {layer_idx}")
            if not hasattr(mglayer.mlp, "shared_experts") or mglayer.mlp.shared_experts is None or args.select_shared_expert_method=='noshare':
                pass
            elif args.select_shared_expert_method == "copy":
                ...
                mglayer.mlp.shared_experts.linear_fc2.weight.copy_(hflayer.mlp.down_proj.weight)
            else:
                raise(NotImplementedError(f"Not Implementation for select_shared_expert_method as {args.select_shared_expert_method}"))
            ########################################
            # Step-2: Initialize router
            ########################################
            print(f"router_init_method: {args.router_init_method}, Layer id: {layer_idx}")
            if args.router_init_method == "normal":
                nn.init.normal_(mglayer.mlp.router.weight, mean=0, std=0.02)
                if hasattr(mglayer.mlp, "shared_experts") and hasattr(mglayer.mlp.shared_experts, "gate_weight") and mglayer.mlp.shared_experts.gate_weight is not None:
                    nn.init.normal_(mglayer.mlp.shared_experts.gate_weight.data, mean=0, std=0.02)
            else:
                raise(NotImplementedError("Not Implementation for router_init_method as {args.router_init_method}"))
            ########################################
            # Step-3: Initialize non-shared expert
            ########################################
            print(f"expert_init_method: {args.expert_init_method}, Layer id: {layer_idx}")
            if args.expert_init_method == 'copy':
                for i in range(args.num_experts):
                    ...
                    linear_fc2_weighti = getattr(mglayer.mlp.experts.linear_fc2, 'weight' + str(i))
                    coeff = 1 #args.num_experts / args.moe_router_topk
                    linear_fc2_weighti.copy_(hflayer.mlp.down_proj.weight*coeff)
            elif args.expert_init_method == "all_normal":
                for i in range(args.num_experts):
                    # print(f"Expert id: {i}")
                    linear_fc1_weighti = getattr(mglayer.mlp.experts.linear_fc1, 'weight' + str(i))
                    nn.init.normal_(linear_fc1_weighti, mean=0, std=0.02)
                    linear_fc2_weighti = getattr(mglayer.mlp.experts.linear_fc2, 'weight' + str(i))
                    nn.init.normal_(linear_fc2_weighti, mean=0, std=0.02)
            elif args.expert_init_method=="finermoe":
                # HIDDEN_SIZE --> MOE_INTERMEDIATE_SIZE --> FINER_HIDDEN_SIZE --> concat --> HIDDEN_SIZE
                # hflayer.mlp.gate_proj.weight.shape: torch.Size([18944, 3584])
                # hflayer.mlp.up_proj.weight.shape: torch.Size([18944, 3584])
                # hflayer.mlp.down_proj.weight.shape: torch.Size([3584, 18944])
                assert args.num_experts % args.G_I % args.G_O == 0
                assert hflayer.mlp.gate_proj.weight.shape[0] % args.G_I == 0
                assert hflayer.mlp.up_proj.weight.shape[0] % args.G_I == 0
                assert args.num_experts % args.G_O == 0
                if not hasattr(mglayer.mlp, "shared_experts") or mglayer.mlp.shared_experts is None:
                    coeff = args.G_I
                else:
                    coeff = 1
                
                if args.R_I == 0:
                    inter_expand_rate = args.G_I
                else:
                    inter_expand_rate = args.R_I

                if args.R_O == 0:
                    out_expand_rate = args.G_O
                else:
                    out_expand_rate = args.R_O
                    
                for i in range(args.num_experts):
                    # print(f"Expert id: {i}")
                    gate_shard_size = hflayer.mlp.gate_proj.weight.shape[0] // args.G_I 
                    up_shard_size = hflayer.mlp.up_proj.weight.shape[0] // args.G_I 
                    num_experts_per_add_group = args.G_I * inter_expand_rate

                    split_gate_weight = hflayer.mlp.gate_proj.weight[((i%num_experts_per_add_group)%args.G_I) * gate_shard_size : (((i%num_experts_per_add_group)%args.G_I)+1) * gate_shard_size ]
                    split_up_weight = hflayer.mlp.up_proj.weight[((i%num_experts_per_add_group)%args.G_I) * up_shard_size : (((i%num_experts_per_add_group)%args.G_I)+1) * up_shard_size ]
                    split_fc1_weight = torch.cat([split_gate_weight, split_up_weight])
                    linear_fc1_weighti = getattr(mglayer.mlp.experts.linear_fc1, 'weight' + str(i))
                    linear_fc1_weighti.copy_(split_fc1_weight)

                    num_experts_per_concat_group = out_expand_rate * args.G_I * inter_expand_rate

                    fc2_in_shard_size = hflayer.mlp.down_proj.weight.shape[1] // args.G_I 
                    fc2_out_shard_size = hflayer.mlp.down_proj.weight.shape[0] // args.G_O 
                    linear_fc2_weighti = getattr(mglayer.mlp.experts.linear_fc2, 'weight' + str(i))
                    linear_fc2_weighti.copy_(coeff * hflayer.mlp.down_proj.weight[(i//num_experts_per_concat_group)*fc2_out_shard_size : ((i//num_experts_per_concat_group)+1)*fc2_out_shard_size, ((i%num_experts_per_add_group)%args.G_I) * fc2_in_shard_size : (((i%num_experts_per_add_group)%args.G_I)+1) * fc2_in_shard_size])
            else:
                raise(NotImplementedError("Not Implementation for expert_init_method as {args.expert_init_method}"))
            ########################################
            # Step-4: Initialize MoE-Concat-Proj
            ########################################
            if not hasattr(mglayer.mlp, "concat_proj"):
                pass
            else:
                print(f"concatproj_init_method: {args.concatproj_init_method}, Layer id: {layer_idx}")
                if args.concatproj_init_method == "normal":
                    nn.init.normal_(mglayer.mlp.concat_proj.weight, mean=0, std=0.02)
                elif args.concatproj_init_method == "zero":
                    nn.init.zeros_(mglayer.mlp.concat_proj.weight)
    
        mgmodel.decoder.final_layernorm.weight.copy_(hfmodel.model.norm.weight)
        if args.untie_embeddings_and_output_weights:
            if hasattr(hfmodel.lm_head, 'weight'): # dense model set untie-embedings-and_output_weights
                mgmodel.output_layer.weight.copy_(hfmodel.lm_head.weight)
            else: # dense model unset untie-embedings-and_output_weights
                mgmodel.output_layer.weight.copy_(hfmodel.model.embed_tokens.weight)
...
def hf_register(hf_ckpt_path):
    ...
    if has_py_file:
        print("There seems exists modeling file")
...
def main():
    ...
    if args.convert_checkpoint_from_megatron_to_transformers:
        ...
        try:
            ...
            print("Can not load pretrained model, trying to load from config")
            hf_model = AutoModelForCausalLM.from_config(config)
        print("HF model loaded")
        mg_model = load_megatron_model(args)
        print("Megatron model loaded")
        convert_from_megatron_to_transformers(mg_model, hf_model, args)
        del mg_model
        gc.collect()
        ...
        mg_model = mg_model.to("cpu")
        print("Upcycling Convert Begin")
        upcycle_checkpoint_from_transformers_to_megatron(hf_model, mg_model, args)
        del hf_model
        gc.collect()
        print("Upcycling Convert End")
